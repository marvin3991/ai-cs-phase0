...
_TAGS = ("GROUNDED", "GENERAL", "HANDOFF", "CHITCHAT")
CHITCHAT_DEFAULT = "您好!出血、解析度、色彩模式、檔案格式等印前問題都可以問我。"


def _parse_tagged(reply: str) -> tuple[str, str]:
    """解析標記。容忍:標記與內容同行(「CHITCHAT 您好」)、標記帶冒號。
    無法解析回 ('INVALID', 原文) → 上層安全降級轉人工。"""
    text = (reply or "").strip()
    if not text:
        return "INVALID", ""
    lines = text.split("\n", 1)
    ...
    tag = parts[0].upper().rstrip(":") if parts else ""
    if tag in _TAGS:
...
def answer(
    ...
    if tag == "CHITCHAT":
        return {"action": "answer", "text": body or CHITCHAT_DEFAULT, "sources": [], "reason": "chitchat"}
